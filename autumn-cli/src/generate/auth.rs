...
fn plan_auth_options_impl(
    ...
    if !oauth.providers.is_empty() {
        ...
        .collect();
        let updated_schema =
            append_schema_table(&schema_base, "oauth_identities", &oauth_fields);
    ...
    if passkeys {
        ...
        .collect();
        let updated_schema =
            append_schema_table(&schema_base, "webauthn_credentials", &wc_fields);
        plan.modify(schema_path, updated_schema);
...
#[allow(clippy::too_many_lines)]
fn render_passkeys_routes_file(
    pascal_name: &str,
    snake_name: &str,
    user_table: &str,
) -> String {
...
mod tests {
    ...
    fn passkey_plan(tmp: &std::path::Path) -> Plan {
        plan_auth_full_ex(tmp, "User", "20260508000000", &AuthOAuthOptions::default(), false, true)
            .unwrap()
    }
    ...
    fn passkeys_flag_defaults_off() {
        ...
        let tmp = project_with_main();
        plan_auth_full(tmp.path(), "User", "20260508000000", &AuthOAuthOptions::default(), false)
            .unwrap()
            .execute(Flags::default())
            .unwrap();
        assert!(
    ...
    fn passkeys_plan_emits_webauthn_credentials_migration() {
        ...
        assert!(up.contains("credential_id"), "missing credential_id column: {up}");
        assert!(up.contains("credential_json"), "missing credential_json column: {up}");
        let down = fs::read_to_string(
            tmp.path()
    ...
    fn passkeys_plan_emits_list_and_revoke_surface() {
        ...
        passkey_plan(tmp.path()).execute(Flags::default()).unwrap();
        let routes = fs::read_to_string(tmp.path().join("src/routes/passkeys.rs")).unwrap();
        assert!(routes.contains("passkey_list"), "missing passkey_list: {routes}");
        assert!(routes.contains("passkey_revoke"), "missing passkey_revoke: {routes}");
        assert!(
            routes.contains("#[secured]"),
    ...
    fn passkeys_plan_emits_rp_config_in_autumn_toml() {
        ...
        passkey_plan(tmp.path()).execute(Flags::default()).unwrap();
        let toml = fs::read_to_string(tmp.path().join("autumn.toml")).unwrap();
        assert!(toml.contains("[auth.webauthn]"), "autumn.toml missing [auth.webauthn]: {toml}");
        assert!(toml.contains("rp_id"), "autumn.toml missing rp_id: {toml}");
        assert!(toml.contains("rp_name"), "autumn.toml missing rp_name: {toml}");
        assert!(toml.contains("rp_origin"), "autumn.toml missing rp_origin: {toml}");
    }
    ...
    fn passkeys_plan_generates_integration_tests() {
        ...
        passkey_plan(tmp.path()).execute(Flags::default()).unwrap();
        let tests =
            fs::read_to_string(tmp.path().join("tests/auth_passkeys.rs")).unwrap();
        for needle in [
            ...
            assert!(tests.contains(needle), "auth_passkeys.rs missing test: {needle}");
        }
    }
    ...
    fn passkeys_plan_registers_routes_in_main() {
        ...
        for entry in [
            ...
            assert!(main.contains(entry), "main.rs missing passkey route: {entry}");
        }
    }
