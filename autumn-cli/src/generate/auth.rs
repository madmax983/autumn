...
pub async fn signup(
    ...
    // Non-enumerating: log send failures but do not surface them to the caller.
    if let Err(e) = send_confirmation_email(&mailer, &{snake_name}.email, &raw_confirm_token).await {{
        tracing::error!("confirmation email failed on signup: {{e}}");
    }}
...
{totp_reset_branch}
    ...
    // If the account is unconfirmed, send the user back to the confirmation page
    // rather than granting an authenticated session.
    if {snake_name}.email_confirmed_at.is_none() {{
