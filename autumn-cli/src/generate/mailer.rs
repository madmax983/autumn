...
pub fn plan_mailer(project_root: &Path, name: &str) -> Result<Plan, GenerateError> {
    ...
    // ── tests/<snake>_mailer.rs ────────────────────────────────────────────
    plan.create(
        project_root
            .join("tests")
            .join(format!("{snake_name}_mailer.rs")),
        render_smoke_test(&struct_name, &snake_name),
    );
...
fn render_smoke_test(struct_name: &str, snake_name: &str) -> String {
    format!(
        "//! Smoke test generated by `autumn generate mailer`.\n\
         //!\n\
...
"#
    ...
    fn plan_creates_smoke_test() {
        ...
        assert!(
            plan.actions
                .iter()
                .any(|a| a.path().ends_with("tests/welcome_mailer.rs")),
            "plan must include tests/welcome_mailer.rs"
        );
    }
