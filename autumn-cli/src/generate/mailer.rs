...
//! - `src/mailers/welcome.rs` — `WelcomeMailer` struct with `#[mailer]` impl and
//!   `#[mailer_preview]` impl. The `#[mailer]` macro generates `send_welcome`
//!   (async) and `deliver_later_welcome` (fire-and-forget) from each method.
//! - `templates/mailers/welcome.html` — HTML template placeholder.
//! - `templates/mailers/welcome.txt` — plain-text template placeholder.
//! - `src/mailers/mod.rs` — created or updated with `pub mod welcome;`.
...
"#
    ...
    fn execute_writes_mailer_struct_and_macro_annotations() {
        ...
        assert!(
            ...
            "must have #[mailer_preview] attribute"
        );
    }
    ...
    fn execute_updates_mailer_mod_rs() {
        ...
        let mod_rs =
            fs::read_to_string(tmp.path().join("src/mailers/mod.rs")).unwrap();
        assert!(
