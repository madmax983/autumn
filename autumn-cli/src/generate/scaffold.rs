...
fn render_repository_file(pascal_name: &str, snake_name: &str) -> String {
    ...
    format!(
        "//! Generated by `autumn generate scaffold`.\n\
         //!\n\
         //! `#[repository]` auto-generates CRUD methods.\n\
         //! API routes are intentionally opt-in: add `api = \"/api/{plural}\"` only\n\
         //! after adding authentication/authorization for your app.\n\
         \n\
         use crate::models::{snake_name}::{{{pascal_name}, New{pascal_name}, Update{pascal_name}}};\n\
         use crate::schema::{plural};\n\
         \n\
         #[autumn_web::repository({pascal_name})]\n\
...
fn main_route_entries(plural: &str, _snake_name: &str) -> Vec<String> {
    vec![
...
"#
    ...
    #[test]
    fn execute_writes_a_repository_without_api_attribute() {
        let tmp = project_with_main(default_main());
