...
fn render_smoke_test(
    ...
    if api {
        ...
        // Build sample JSON values for the fields (same logic as the api path above).
        let mut sample_parts = Vec::new();
        for f in fields {
            let val = match f.kind {
                FieldKind::String | FieldKind::Text => "\\\"test\\\"",
        ...
        format!(
            "//! Smoke test generated by `autumn generate scaffold`.\n\
             ...
             fn {plural}_delete_round_trip_when_server_is_running() {{\n\
                 ...
                 \n\
                 // 1. Create a row via the JSON API (no auth required).\n\
                 let payload = \"{sample_json}\";\n\
                 let (headers, body) = request(host_port, \"POST\", \"/api/{plural}\", Some(payload));\n\
                 assert!(\n\
                     headers.starts_with(\"HTTP/1.1 201\") || headers.starts_with(\"HTTP/1.0 201\")\n\
                         || headers.starts_with(\"HTTP/1.1 200\") || headers.starts_with(\"HTTP/1.0 200\"),\n\
                     \"POST /api/{plural} failed:\\n{{headers}}\\n{{body}}\"\n\
                 );\n\
                 let json: autumn_web::reexports::serde_json::Value =\n\
                     autumn_web::reexports::serde_json::from_str(&body)\n\
                         .unwrap_or_else(|_| panic!(\"failed to parse POST response: {{body}}\"));\n\
                 {id_capture}\n\
                 \n\
                 // 2. Delete via the HTML route — POST /{plural}/{{id}}/delete.\n\
                 // In test environments without CSRF middleware the handler\n\
                 // redirects (302/303). With CSRF enforced it returns 403, which\n\
                 // still confirms the route is wired; skip the 404 assertion then.\n\
                 let delete_path = format!(\"/{plural}/{{id}}/delete\");\n\
                 let (del_headers, _del_body) = request(host_port, \"POST\", &delete_path, Some(\"\"));\n\
                 let delete_succeeded =\n\
                     del_headers.starts_with(\"HTTP/1.1 302\") || del_headers.starts_with(\"HTTP/1.0 302\")\n\
                     || del_headers.starts_with(\"HTTP/1.1 303\") || del_headers.starts_with(\"HTTP/1.0 303\");\n\
                 assert!(\n\
                     delete_succeeded || del_headers.contains(\" 403\") || del_headers.contains(\" 200\"),\n\
                     \"POST {{delete_path}} expected redirect or 403: {{del_headers}}\"\n\
                 );\n\
                 \n\
                 // 3. Verify the row is gone via the JSON API (only when the\n\
                 // delete redirected, meaning CSRF was not enforced in this env).\n\
                 if delete_succeeded {{\n\
                     let item_path = format!(\"/api/{plural}/{{id}}\");\n\
                     let (get_headers, get_body) = request(host_port, \"GET\", &item_path, None);\n\
                     assert!(\n\
                         get_headers.starts_with(\"HTTP/1.1 404\") || get_headers.starts_with(\"HTTP/1.0 404\"),\n\
                         \"GET {{item_path}} after delete did not return 404:\\n{{get_headers}}\\n{{get_body}}\"\n\
                     );\n\
                 }}\n\
             }}\n",
...
"#
    ...
    fn smoke_test_delete_round_trip_exercises_create_and_delete() {
        ...
        plan.execute(Flags::default()).unwrap();
        let test = fs::read_to_string(tmp.path().join("tests/post.rs")).unwrap();
        // Round-trip test: creates via JSON API, deletes via HTML route, verifies 404.
        assert!(
            test.contains("posts_delete_round_trip_when_server_is_running"),
