...
fn render_routes_file(
    ...
    // When attachment fields are present, forms need multipart encoding.
...
fn render_update_columns(plural: &str, fields: &[Field]) -> String {
    use std::fmt::Write;
    // Attachment fields are excluded: updating a blob attachment requires
    // a separate multipart upload flow, not inline form data.
    let non_attachment: Vec<&Field> = fields
        .iter()
        .filter(|f| !f.kind.is_attachment())
        .collect();
    // Estimate 50 chars per field
    let mut out = String::with_capacity(non_attachment.len() * 50);
    for (i, f) in non_attachment.iter().enumerate() {
        if i > 0 {
            out.push_str(", ");
        }
        write!(
