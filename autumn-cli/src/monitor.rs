...
fn draw_routes_tab(frame: &mut ratatui::Frame, area: Rect, state: &DashboardState) {
    ...
    routes.sort_by_key(|route| std::cmp::Reverse(route.1.count));
...
mod tests {
    ...
    fn test_poll_updates_state() {
        // Start a mock server
        let listener = TcpListener::bind("127.0.0.1:0").expect("failed extracting response component");
        let port = listener.local_addr().expect("failed extracting response component").port();
    ...
    fn deserialize_health_response() {
        ...
        let health: HealthResponse = serde_json::from_str(json).expect("failed extracting response component");
    ...
    fn deserialize_health_with_db_check() {
        ...
        let health: HealthResponse = serde_json::from_str(json).expect("failed extracting response component");
        let db = health.checks.expect("failed extracting response component").database.expect("failed extracting response component");
    ...
    fn deserialize_metrics_response() {
        ...
        let metrics: MetricsResponse = serde_json::from_str(json).expect("failed extracting response component");
    ...
    fn deserialize_metrics_with_db() {
        ...
        let metrics: MetricsResponse = serde_json::from_str(json).expect("failed extracting response component");
        let db = metrics.database.expect("failed extracting response component");
        assert_eq!(db.pool_size, 10);
    ...
    fn deserialize_tasks_response() {
        ...
        let tasks: TasksResponse = serde_json::from_str(json).expect("failed extracting response component");
    ...
    fn deserialize_loggers_response() {
        ...
        let loggers: LoggersResponse = serde_json::from_str(json).expect("failed extracting response component");
    ...
    fn deserialize_channels_response() {
        ...
        let channels: ChannelsResponse = serde_json::from_str(json).expect("failed extracting response component");
    ...
    fn render_frame(state: &DashboardState, width: u16, height: u16) {
        ...
        terminal.draw(|frame| draw(frame, state)).expect("failed extracting response component");
    }
