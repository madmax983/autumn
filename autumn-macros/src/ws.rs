...
mod tests {
    ...
    #[test]
    fn test_is_app_state_type() {
        let ty: syn::Type = syn::parse_str("AppState").unwrap();
        assert!(is_app_state_type(&ty));

        let ty: syn::Type = syn::parse_str("autumn_web::AppState").unwrap();
        assert!(is_app_state_type(&ty));

        let ty: syn::Type = syn::parse_str("NotAppState").unwrap();
        assert!(!is_app_state_type(&ty));
    }

    #[test]
    fn test_ws_macro_valid_input() {
        let attr = quote! { "/ws" };
        let item = quote! {
            pub async fn ws_handler(state: AppState, user: User) -> impl WsHandler {
                |mut socket: WebSocket| async move { }
            }
        };

        let result = ws_macro(attr, item);
        let result_str = result.to_string();

        assert!(result_str.contains("__autumn_ws_upgrade_ws_handler"));
        assert!(result_str.contains("__autumn_route_info_ws_handler"));
        assert!(result_str.contains("on_upgrade"));
        assert!(result_str.contains("pub async fn __autumn_ws_upgrade_ws_handler"));
        assert!(result_str.contains("user : User"));
        assert!(result_str.contains("ws_handler (__autumn_state . clone () , user)"));
    }
}
