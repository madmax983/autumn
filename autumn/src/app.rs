...
fn build_router_inner_with_env(
    ...
    if dev_reload_enabled {
        ...
        let env_state: std::sync::Arc<dyn crate::config::Env + Send + Sync> =
            if let Some(mock) = env.as_any().downcast_ref::<crate::config::MockEnv>() {
...
mod tests {
    ...
    async fn build_router_injects_live_reload_script_when_enabled() {
        ...
        let env = MockEnv::new().with("AUTUMN_DEV_RELOAD", "1").with(
            "AUTUMN_DEV_RELOAD_STATE",
            reload_file.path().to_str().expect("utf-8 path"),
        );
        ...
        let router = build_router_with_env(
            vec![Route {
                method: http::Method::GET,
                path: "/page",
    ...
    async fn build_router_mounts_dev_reload_endpoint_when_enabled() {
        ...
        let router = build_router_with_env(
            vec![test_get_route("/dummy", "dummy")],
            &config,
            state,
            &env,
        );
