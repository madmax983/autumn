...
///
/// Trait to abstract the state requirement for authorization functions.
/// This breaks the circular dependency between the authorization module
/// and the central `AppState`.
pub trait AuthorizationState {
    /// Returns the configured policy registry.
    fn policy_registry(&self) -> &PolicyRegistry;

    /// Returns the configured forbidden response shape.
...
impl PolicyContext {
    ...
    pub async fn from_request(
        state: &(impl AuthorizationState + Send + Sync + ?Sized),
        session: &Session,
    ) -> Self {
