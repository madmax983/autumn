...
//! - [`UpdateDraft<T>`] -- merged before/after snapshot of a model, used
//!   by `#[model]`-generated `before_update` hooks.
//! - [`DraftField<'a, T>`] -- borrowing per-field accessor into an
//!   `UpdateDraft`, enabling ergonomic `draft.field().changed_to(...)`.
