...
mod sns_verify {
    ...
    pub(super) fn verify_rsa_signature(
        ...
        let public_key = rsa::RsaPublicKey::from_public_key_der(spki_der).map_err(|_| ())?;        if sig_version == "2" {
...
fn extract_multipart_bodies(
    ...
    for part in raw_parts.into_iter().skip(1) {
        ...
        let part_ct_lower = part_headers
            ...
            .map_or_else(|| "text/plain".to_string(), |l| l[13..].trim().to_ascii_lowercase());
    ...
    loop {
        ...
        if let Some(filename) = filename {
            ...
            let part_ct = headers_str
                .lines()
                .find(|l| l.to_ascii_lowercase().starts_with("content-type:"))
                .map_or_else(|| "application/octet-stream".to_string(), |l| {
                    l[13..]
                        .trim()
                        .split(';')
                        .next()
    ...
    let handler = move |_headers: HeaderMap, body: Bytes| {
        ...
        async move {
            ...
            if let Ok(json) = serde_json::from_slice::<serde_json::Value>(&body) {
                ...
                if let Err(status) = sns_verify::verify(&json, &http_client, topic_arn.as_deref()).await {
                    return status;
                }
            }
...
mod tests {
    ...
    fn parse_ses_notification_preserves_envelope_recipient_casing() {
        ...
        let email = match result.unwrap() {
            ...
            other @ SnsParseResult::SubscriptionConfirmation { .. } => panic!("expected Email, got {other:?}"),
        };
        assert!(
