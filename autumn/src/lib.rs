...
/// autumn::app().routes(all_routes).run().await;
/// # }
/// ```
...
/// them as `autumn::reexports::axum`, etc. without requiring the user to
...
/// `autumn::reexports::axum::Router` for custom middleware).
///
/// # Available crates
///
/// | Crate | Re-exported as | Use case |
/// |-------|---------------|----------|
/// | `axum` | `autumn::reexports::axum` | Custom routers, middleware, extractors |
/// | `diesel` | `autumn::reexports::diesel` | Raw Diesel queries, schema types |
/// | `http` | `autumn::reexports::http` | HTTP types (`StatusCode`, `Method`, headers) |
/// | `tokio` | `autumn::reexports::tokio` | Async runtime, spawn, timers |
pub mod reexports {
    pub use axum;
    #[cfg(feature = "db")]
    pub use diesel;
