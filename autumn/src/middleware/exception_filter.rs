    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match this.inner.poll(cx) {
            Poll::Ready(Ok(mut response)) => {
                // If the response is an error but doesn't have AutumnErrorInfo yet,
                // generate one so that exception filters (like the error page filter) can run.
                if (response.status().is_client_error() || response.status().is_server_error())
                    && response.extensions().get::<AutumnErrorInfo>().is_none()
                {
                    let reason = response
                        .status()
                        .canonical_reason()
                        .unwrap_or("Error")
                        .to_string();
                    let error_info = AutumnErrorInfo {
                ...
                if let Some(error_info) = response.extensions().get::<AutumnErrorInfo>().cloned() {
                    ...
                    // If the response is still empty (e.g. from an Axum fallback or default response)
                    // but it's an error, convert it to a JSON error response unless it's a HEAD request
                    // or it already has a content body (which could be an HTML error page from the ErrorPageFilter).
                    // We only do this if no HTML page was generated by ErrorPageFilter.
                    let is_empty =
                        axum::body::HttpBody::size_hint(response.body()).exact() == Some(0);
                    let is_error =
                        response.status().is_client_error() || response.status().is_server_error();
                    let is_json_requested = !response
                        .extensions()
                        .get::<crate::middleware::error_page_filter::WantsHtml>()
                        .map_or(false, |w| w.0);

                    if is_empty && is_error && is_json_requested {
                        let json_body = error_info.clone().into_default_response();
                        let (parts, _) = response.into_parts();
                        let mut new_response = json_body;
                        ...
                        *new_response.headers_mut() = parts.headers;
                        ...
                        // Make sure we have the right content length
                        if let Some(cl) =
                            axum::body::HttpBody::size_hint(new_response.body()).exact()
                        {
                            new_response.headers_mut().insert(
                                axum::http::header::CONTENT_LENGTH,
                                axum::http::HeaderValue::from(cl),
                            );
                        }

                        Poll::Ready(Ok(new_response))
                    } else {
