...
fn build_router_pre_state(
    ...
    let router = if let Some((mount_path, tools, endpoint_layer)) = mcp_prepared {
        ...
        let wiring = crate::mcp::McpWiring {
            ...
            trusted_hosts: crate::security::TrustedHostPolicy::from_config(config),
            tenant_header,
...
fn apply_middleware(
    ...
    let trusted_host_policy = crate::security::TrustedHostPolicy::from_config(config);
...
async fn trusted_host_middleware(
    ...
    next: Next,
    policy: crate::security::TrustedHostPolicy,
) -> axum::response::Response {
    ...
    let parsed_host = raw_host.and_then(crate::security::extract_host_without_port);
