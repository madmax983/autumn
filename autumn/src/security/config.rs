...
impl ResolvedSigningKeys {
    ...
    pub fn verify(&self, message: &[u8], hex_sig: &str) -> bool {
        if crate::security::constant_time_eq_str(&hmac_sha256_hex(&self.current, message), hex_sig) {
            return true;
        }
        for prev in &self.previous {
            if crate::security::constant_time_eq_str(&hmac_sha256_hex(prev, message), hex_sig) {
                return true;
            }
        }
        false
