...
async fn verify_csrf_token(
    ...
    if let (Some(c), Some(h)) = (cookie_token, header_token)
        ...
        && crate::security::constant_time_eq_str(c, h)
    {
        token_found = true;
    }
    ...
    if let (Some(c), Some(q)) = (cookie_token, &query_token)
        ...
        && !q.is_empty()
        && validate_cookie_token_hmac(c, settings)
        && crate::security::constant_time_eq_str(c, q)
    {
        token_found = true;
    }
    ...
    if is_urlencoded {
        for (key, value) in url::form_urlencoded::parse(&bytes) {
            if key == settings.form_field {
                if let Some(c) = cookie_token
                    ...
                    && !value.is_empty()
                    && validate_cookie_token_hmac(c, settings)
                    && crate::security::constant_time_eq_str(c, value.as_ref())
                {
                    token_found = true;
                }
                break;
        ...
        if let Some(value) = scan_multipart_field(&bytes, boundary, &settings.form_field) {
            if let Some(c) = cookie_token
                ...
                && !value.is_empty()
                && validate_cookie_token_hmac(c, settings)
                && crate::security::constant_time_eq_str(c, value)
            {
                token_found = true;
            }
        }
