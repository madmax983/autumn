//! Middleware for serving generated static files and handling ISR.
//!
//! The [`StaticFileLayer`] intercepts incoming HTTP requests before they reach
//! the dynamic router. If the requested URL path matches an entry in the
//! `manifest.json` (generated by `autumn build`), the middleware serves the
//! pre-rendered HTML file directly from the `dist/` directory.
//!
//! If Incremental Static Regeneration (ISR) is configured for the route (via `revalidate`),
//! this middleware also checks the age of the cached file. If the file is stale,
//! it serves the stale content immediately to the current user, but kicks off a
//! background Tokio task to re-render the page and atomically update the file
//! for future visitors.
