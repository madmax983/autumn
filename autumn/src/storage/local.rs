...
pub fn verify_upload_with_now(
    ...
    if crate::security::constant_time::constant_time_eq_str(&expected, signature) {
        return Ok(());
    }
    let expected_legacy = sign_upload_legacy(signing_key, blob_key, content_type, expires_at);
    if crate::security::constant_time::constant_time_eq_str(&expected_legacy, signature) {
        return Ok(());
    }
...
pub(crate) fn verify_upload_rotation_with_now(
    ...
    if crate::security::constant_time::constant_time_eq_str(&expected_current, signature) {
        return Ok(());
    }
    let expected_current_legacy =
        sign_upload_legacy(current.as_bytes(), blob_key, content_type, expires_at);
    if crate::security::constant_time::constant_time_eq_str(&expected_current_legacy, signature) {
        return Ok(());
    }
