...
async fn extract_tenant_from_session(
    parts: &axum::http::request::Parts,
    config: &crate::config::AutumnConfig,
) -> Result<String, crate::AutumnError> {
    let session = parts
...
fn extract_tenant_from_jwt(
    ...
    let auth_header = parts
        ...
        .ok_or_else(|| {
            crate::AutumnError::unauthorized_msg("Missing Authorization header for JWT tenancy")
        })?;
    ...
    let secret = config
        ...
        .jwt_secret
        .as_ref()
        .ok_or_else(|| crate::AutumnError::unauthorized_msg("JWT secret is not configured"))?;
    ...
    .map_err(|e| crate::AutumnError::unauthorized_msg(format!("JWT validation failed: {e}")))?;
...
pub async fn extract_tenant_from_parts(
    ...
    match config.tenancy.source.as_str() {
        "header" => extract_tenant_from_header(parts, config),
        "subdomain" => extract_tenant_from_subdomain(parts, config),
        "session" => extract_tenant_from_session(parts, config).await,
        "jwt" => extract_tenant_from_jwt(parts, config),
