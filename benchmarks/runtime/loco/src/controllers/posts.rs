...
fn validate_create(p: &CreatePost) -> Result<(), &'static str> {
    ...
    if p.body.trim().is_empty() {
        return Err("body must not be blank");
    }
    if p.author.trim().is_empty() {
        return Err("author must not be blank");
    }
    Ok(())
}
...
pub async fn show(State(ctx): State<AppContext>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match PostEntity::find_by_id(id).one(&ctx.db).await {
        Ok(Some(existing)) => {
            ...
            if let Some(t) = body.title {
                am.title = ActiveValue::Set(t);
            }
            if let Some(b) = body.body {
                am.body = ActiveValue::Set(b);
            }
            if let Some(p) = body.published {
                am.published = ActiveValue::Set(p);
            }
            if let Some(a) = body.author {
                am.author = ActiveValue::Set(a);
            }
...
pub async fn delete(State(ctx): State<AppContext>, Path(id): Path<i64>) -> impl IntoResponse {
    match PostEntity::find_by_id(id).one(&ctx.db).await {
        Ok(Some(p)) => {
...
pub async fn protected(State(ctx): State<AppContext>, headers: HeaderMap) -> impl IntoResponse {
    ...
    match token_row {
        Ok(Some(t)) => {
            ...
            Json(serde_json::json!({ "principal": t.principal, "total_posts": total }))
                .into_response()
        }
        _ => (
            StatusCode::UNAUTHORIZED,
...
pub async fn html_show(State(ctx): State<AppContext>, Path(id): Path<i64>) -> impl IntoResponse {
    match PostEntity::find_by_id(id).one(&ctx.db).await {
        Ok(Some(p)) => {
