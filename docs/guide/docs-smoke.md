...
Autumn 0.3.x line. It must use Rust 1.88.0+ and the published `autumn-cli` and
...
```text
docs-smoke: workspace-prepublish rehearsal because autumn-cli/autumn-web 0.3.0
were not yet available on crates.io.
```

That rehearsal does not certify the release. The published docs-smoke must be
rerun with `cargo install autumn-cli --version 0.3.0` and no workspace patches
